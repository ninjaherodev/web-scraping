...
if response.status_code == 200:
    ...
    headers = soup.find("section", class_="block_full row-content row_box row_number_4").find('table').find('thead').find_all('th')
    headersFormat = [header.text for header in headers[:-1]]
    print(headersFormat)

    datos = soup.find("section", class_="block_full row-content row_box row_number_4").find('table').find('tbody').find_all('tr', class_='tab-pane')
